@verbatim
<style>
    .toast-slide-enter-active { transition: all 0.3s cubic-bezier(0.4,0,0.2,1); }
    .toast-slide-leave-active { transition: all 0.2s cubic-bezier(0.4,0,0.2,1); }
    .toast-slide-enter-from, .toast-slide-leave-to { transform: translateY(-32px); opacity: 0; }
</style>
<transition
    enter-active-class="toast-slide-enter-active"
    enter-from-class="toast-slide-enter-from"
    leave-active-class="toast-slide-leave-active"
    leave-to-class="toast-slide-leave-to"
>
    <div v-if="notifications && notifications.length > 0" @click="removeNotification(notifications[0].id)"
        :class="[
            ...
            notifications[0].type === 'warning' ? 'bg-yellow-100 border-yellow-500 text-yellow-700 dark:bg-yellow-900 dark:border-yellow-700 dark:text-yellow-100' : '',
        ...
        <div class="flex items-center">
            ...
            <i v-if="notifications[0].type === 'warning'" class="fas fa-exclamation-triangle text-yellow-500 dark:text-yellow-300 mr-3"></i>
            <div>
...
<header class="fixed top-0 left-0 right-0 h-[70px] bg-white dark:bg-gray-700 shadow-md z-40 flex items-center px-4">
    <div class="flex items-center justify-between w-full">
        ...
        <div class="flex items-center space-x-4 ml-auto">
            <div class="relative">
                <button id="employer-notif-bell" type="button" class="relative text-gray-600 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 transition-colors" title="Notifications">
                    <i class="fas fa-bell text-xl"></i>
                    <span id="employer-notif-badge" class="hidden absolute -top-1 -right-1 bg-red-500 text-white text-[10px] font-bold rounded-full min-w-[16px] h-4 flex items-center justify-center px-1"></span>
                </button>
                <div id="employer-notif-panel" class="hidden absolute right-0 mt-2 w-80 max-w-[90vw] bg-white dark:bg-gray-600 rounded-md shadow-lg z-50 border border-gray-200 dark:border-gray-500 origin-top-right">
                    <div class="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-gray-500">
                        ...
                        <button id="employer-notif-mark-all" type="button" class="text-xs text-blue-600 dark:text-blue-400 hover:underline">Mark all as read</button>
                    </div>
                    <div id="employer-notif-list" class="max-h-96 overflow-y-auto divide-y divide-gray-100 dark:divide-gray-500">
                        <div class="px-4 py-6 text-center text-sm text-gray-400">Loading...</div>
                    </div>
                    <a href="employer_notification" class="block text-center py-2.5 text-sm font-medium text-blue-600 dark:text-blue-400 hover:bg-gray-50 dark:hover:bg-gray-500 border-t border-gray-200 dark:border-gray-500">See more</a>
                </div>
            </div>
            <div class="relative profile-dropdown-wrapper">
                <div class="cursor-pointer flex items-center" @click.stop="toggleProfileDropdown">
                    <img :src="employerProfile.company_logo || '<?= asset('assets/images/logo-sm.png') ?>'" alt="Profile" class="w-10 h-10 rounded-full border-2 border-gray-200 dark:border-gray-500">
...
<script>
    (function () {
        ...
        var ROUTE = 'employer_notification';

        function el(id) { return document.getElementById('employer-notif-' + id); }
        ...
        function targetFor(n) {
            if (n.type === 'new_application') return 'employer_applicants';
            if (n.type === 'password') return 'employer_settings';
            if (n.type === 'message' && n.message_id) return 'employer_messages?message_id=' + n.message_id;
            return null;
        }
