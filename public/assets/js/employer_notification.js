    ...
    methods: {
        ...
        async fetchEmployerProfile() {
            try {
                ...
                this.employerProfile = data.success ? data.profile : { company_logo: null, company_name: 'Employer' };
            } catch (error) {
                this.employerProfile = { company_logo: null, company_name: 'Employer' };
            }
        },
        ...
        async fetchNotifications() {
            try {
                this.loading = true;
                const res = await fetch('employer_notification?action=list');
                const data = await res.json();
        ...
        async markNotificationAsRead(notification) {
            if (!notification.read) {
                ...
                try {
                    await fetch('employer_notification?action=update', {
                        method: 'POST',
        ...
        notificationTarget(notification) {
            if (notification.type === 'new_application') return 'employer_applicants';
            if (notification.type === 'password') return 'employer_settings';
            if (notification.type === 'message' && notification.message_id) return 'employer_messages?message_id=' + notification.message_id;
            return null;
        },
        markAllAsRead() {
            this.notificationsList.forEach(n => n.read = true);
            fetch('employer_notification?action=update', {
                method: 'POST',
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
